...
model_gen=function(modelset,file)
{
  ...
  for (model in modelset)
  {
    ...
    model_id=model_id+1
    write(paste(' [submodel] ',paste(model,collapse=' '),'\n [rates] ',I,' ',A,' 0','\n [indelmodel] POW 1.5 10\n [indelrate] ', runif(1,0.01,0.25)),file,append=T)
...
indelib_gen=function(n_taxa,n_sim,aln_length) # n_sim = number of simulations per topology
{
  ...
  MODEL=model_gen(modelset,'control.txt')
  ...
  write.table(data.frame('[PARTITIONS]',PNAME,"[",ID_TREE,MODEL,aln_length,"]"),'control.txt',append=T,quote=F,row.names=F,col.names =F)
